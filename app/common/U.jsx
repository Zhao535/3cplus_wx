...
var U = (function () {
    ...
    let price = (() => {
        ...
        let secSpec = (cart = {}, realTimeSecKill = []) => {
            let now = new Date().getTime();
            let { product = {}, productSno = '', cartPayload = {} } = cart;
            let { type } = cartPayload;
            let { specs = [] } = product;
            let _specs = {};

            if (type === 1) {
                _specs = specs.find(spe => spe.sno === productSno) || {};
            }
            if (type === 2) {
                let _realSecKill = realTimeSecKill.find(item => item.id === cartPayload.id) || {};
                let { secKillSpec = [], endAt } = _realSecKill;
                if (endAt < now) {
                    _specs = specs.find(spe => spe.sno === productSno) || {};
                } else {
                    let { specs = [] } = _realSecKill ? _realSecKill.product || [] : [];
                    console.log(specs);
                    _specs = specs.find(spe => spe.sno === productSno) || {};
                    _specs.price = secKillSpec[0] ? secKillSpec[0].price : 0;
                }
            }
        ...
        let cent2yuan = (price, withSymbol) => {
            ...
            return withSymbol ? '￥' + ret : '' + ret; // withSymbol为true￥后要加ret bug
        };
        let yuan2cent = (price) => {
            let ret = 0;
            if (!isNaN(price)) {
        ...
        return {
            formatPrice, cent2yuan, yuan2cent, secSpec
        };
